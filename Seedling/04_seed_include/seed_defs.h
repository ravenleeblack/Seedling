...
enum {
    ...
    _vine_ptr, _passage_ptr, _table_ptr,                                           // Stream pointers
    _address, _fetch,
    ...
    _transfer,   //transfer a smaller reg into larger register with movx
    ...
    _load_elem,     // new trying to make type for arrays
    _load_compute,  // lea - load effective address
    _load_den,      // loadsb - den/ 1 byte
    _load_bay,      // loadsb - bay/ 2 bytes
    _load_aisle,    // loadsb - aisle/ 4 bytes
    _load_zone,     // loadsb - zone/ 8 bytes

    _load_num,      // fild - load integer  convert to floating point
    _load_decimal,  // load floating point value

    _move_deci,     // movss - scalar single precision floating point 32 bit float into SSE
    _move_decii,    // movsd - scalar single precision floating point 64 bit float into SSE
    _move_deciv,    // movups - move unaligned/aligned packed single precision 128 bit float into SSE
    _move_int,      // movd - load interger data into mmx or SSE
    ...
    _ptr, _rel, _align, _org, _bits, _equ, _times,

    // File management
    _use, _replace, _start_use, _get_use, _end_use,
    _start_link, _get_link, _end_link, _start_log, _get_log,
...
struct token 
{
    ...
    char mark_buffer[256];  // Dedicated buffer for mark literals
};
...
typedef struct 
{
    ...
    /*FETCH VALUES*/
    int fetch_expression;                 // For expressions like "esp - 4"
    int fetch_offset_sign;                // 1 for "+", -1 for "-", 0 for none

    /*LOG VALUES*/
    char *phrase_log_value;               // Struct field (e.g., "age")
    ...
    /*TABLE VALUES*/
    char *table_name;
    int phrase_table_count;               // Number of elem values in the array

    /*STAND VALUES*/
    char *phrase_string_value;                  // The string literal, e.g., "World"

    /*MARK VALUES*/
    char * phrase_mark_value;
    char phrase_mark_buffer[2];
