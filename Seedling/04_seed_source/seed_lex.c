...
// Scan a mark literal into buf, return length
int scan_mark(char *buf, int lim) 
{
    int i = 0, c;

    if (!buf || lim <= 1) {
        error("Invalid buffer or limit in scan_mark");
        return 0;
    }
    ...
    while (i < lim - 1) {
        c = next_ch();
        if (c == '\'' || c == EOF) {
            return_char(c);  // Put back the closing quote for outer check
            break;
        }
        buf[i++] = c;
    }
    ...
    switch (*s)
	 {
        ...
        case 'd': 
            ...
            if (!strcmp(s, "dec_den"))     return _dec_den;
            if (!strcmp(s, "dec_dens"))    return _dec_dens;
		    if (!strcmp(s, "dec_bay"))     return _dec_bay;
            if (!strcmp(s, "dec_bays"))    return _dec_bays;
		    if (!strcmp(s, "dec_aisle"))   return _dec_aisle;
            if (!strcmp(s, "dec_aisles"))  return _dec_aisles;
		    if (!strcmp(s, "dec_zone"))    return _dec_zone;
            if (!strcmp(s, "dec_zones"))   return _dec_zones;
		    break;
        ...
        case 'e': 
            ...
            if (!strcmp(s, "end"))            return _end;
            if (!strcmp(s, "end_log"))            return _end_log;
            if (!strcmp(s, "end_list"))            return _end_list; 
		    break;
        case 'f':
        ...
        case 'i':
            ...
            if (!strcmp(s, "index")) return _index;
		    break;
        ...
        case 'l': 
		    ...
		    if (!strcmp(s, "log_sect"))         return _log_section;
            if (!strcmp(s, "list"))         return _list;
            if (!strcmp(s, "list_sect")) return _list_section;
			...
			if (!strcmp(s, "load_elem"))   return _load_elem;
        ...
        case 'm': 
            ...
            if (!strcmp(s, "move_aisles"))   return _move_aisles;
            if (!strcmp(s, "move_zones"))    return _move_zones;
            if (!strcmp(s, "mark_value"))    return _mark_value;
            break;
...
int scan(struct token *t) 
{
    ...
    switch (c)
    {
        ...
        case '\'':
            t->token_rep = _mark_literal;
        
            int len = scan_mark(t->mark_buffer, sizeof(t->mark_buffer));
            if (len != 1) {
                error("Mark literal must be exactly one character");
                return 0;
            }
        
            int closer = next_ch();
            if (closer != '\'') {
                error("Unterminated mark literal");
                return 0;
            }
        
            t->mark_buffer[1] = '\0'; // Ensure null termination
            t->mark_value = t->mark_buffer;
        break;
